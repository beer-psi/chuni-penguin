...
def _render_b30_template(
    ...
    cache_parts = [
        101,  # version number for when constants like fonts, etc. need to be updated
        record_slots,
        new_record_slots,
        rating_title,
        footer_text,
        invite_link,
        (ASSETS_DIR / "b50" / "b50_bg_ongeki.webp").stat().st_mtime,
        (ASSETS_DIR / "b50" / "b50_overlay.webp").stat().st_mtime,
        (ASSETS_DIR / "b50" / "b50_part_header.webp").stat().st_mtime,
        (ASSETS_DIR / "b50" / "b50_logo_ongeki.webp").stat().st_mtime,
    ...
    platinum_row_num = ceil(platinum_record_slots / 5)
    new_row_num: int | None = (
        ceil(new_record_slots / 5) if new_record_slots is not None else None
    )
    ...
    b30_image_height = (
        ...
        + B30_HEADER_SPACING
        + B30_OLD_NEW_SPACING
        + (B30_ENTRY_HEIGHT + B30_ENTRY_HEIGHT_SPACING) * (row_num + platinum_row_num)
        + B30_FOOTER_SPACING
        + B30_FOOTER_HEIGHT
    )
    ...
    if new_record_slots is not None:
        assert new_row_num is not None
    ...
    with closing(
        _make_background_image(
            ASSETS_DIR / "b50" / "b50_bg_ongeki.webp", B30_IMAGE_WIDTH, b30_image_height
        )
    ) as im:
        b30_image = im.copy()
    ...
    # draw logo
    logo_path = ASSETS_DIR / "b50" / "b50_logo_ongeki.webp"
    ...
    # draw the "OLD CHARTS", "NEW CHARTS" and "PLATINUM CHARTS" separators
    with (
        ...
        with (
            ...
            b30_image = _paste_alpha_composite(
                ...
                (
                    ...
                    B30_HEADER_HEIGHT
                    + B30_HEADER_SPACING
                    + row_num * (B30_ENTRY_HEIGHT + B30_ENTRY_HEIGHT_SPACING)
                    + B30_ENTRY_HEIGHT_SPACING,
                ),
            )
    ...
    with (
        ...
        b30_image = _paste_alpha_composite(
            ...
            (
                ...
                + B30_HEADER_SPACING
                + (
                    (row_num + (new_row_num or 0))
                    * (B30_ENTRY_HEIGHT + B30_ENTRY_HEIGHT_SPACING)
                )
                + B30_ENTRY_HEIGHT_SPACING
                + (B30_OLD_NEW_SPACING if new_row_num else 0),
            ),
        )
    ...
    b30_draw.text(
        (40, 316),
        "OLD CHARTS" if new_record_slots is not None else "BEST CHARTS",
        fill="#FFFFFF",
        font=INTER_40_BOLD,
    )
    ...
    return row_num, new_row_num, b30_image
...
def _render_b30_entry(
    ...
    b30_draw.text(
        (x + B30_ENTRY_WIDTH - 10 - rating_text_length, y + 125),
        rating_text,
...
def render_b30(
    *,
    ...
    platinum_records: "Sequence[Score] | None" = None,
    platinum_record_slots: int = 50,
    ...
    row_num, new_row_num, b30_image = _render_b30_template(
        record_slots,
        platinum_record_slots,
    ...
    new_average = 0
    platinum_average = floor_to_ndp(total_platinum_rating / platinum_record_slots, 4)
    ...
    if platinum_records is not None:
        ...
        b30_draw.text(
            ...
            f"{platinum_average:.4f}",
            fill="#000000",
            font=INTER_44_BOLD,
        )

        for i, record in enumerate(platinum_records):
            x = 30 + (i % 5) * (B30_ENTRY_WIDTH + B30_ENTRY_WIDTH_SPACING)
