# chuni penguin

chuni penguin is a [Discord](https://discord.com) bot for
[CHUNITHM International version](https://chunithm.sega.com).

[Invite the bot!](https://chunithm.beerpsi.cc/invite)

## Share your scores

Quickly share your scores in Discord. Show your top scores in a beautiful image
breakdown. No more screenshots!

<figure markdown class="grid">
  ![Image of a recent score shared through the bot](assets/images/recent-score.png)

  ![best50 breakdown](assets/images/best50.png)
</figure>


## Find information quickly

Search for songs, charts and courses. View detailed information about them.

<figure markdown class="grid">
  ![Song information card](assets/images/song-information.png)

  ![Course list](assets/images/course-list.png)
</figure>

## Useful tools

Many other useful tools are included:

![Anmitsu checker output](assets/images/anmitsu-checker.png)
/// caption
Anmitsu checker
///

<figure markdown="span">
  <div markdown class="grid">
![Rating and over power calculator](assets/images/rating-overpower-calculator.png)
![Rank border calculator](assets/images/rank-border-calculator.png)
  </div>
  <figcaption>Calculators</figcaption>
</figure>

![](assets/images/random-charts.png){: width="40%"}
/// caption
Random chart selection
///

![](assets/images/chart-view.png)
/// caption
Chart view (powered by [sdvx.in](https://sdvx.in/chunithm.html))
///

## Fun and games

Ruin your friendships by playing the jacket art and music guessing minigames!

![](assets/images/night-wasted-to-cguess.png)
