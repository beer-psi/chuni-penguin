...
from utils import did_you_mean_text, json_loads, shlex_split
...
def render_b30(
    ...
    b30_draw.rectangle((0, 0, b30_image.width, 124), fill="#7ea3ff")
    ...
    credits_bbox = b30_draw.multiline_textbbox(
        (0, 0),
        "Generated by chuni penguin#3217 as Derakkuma\nhttps://chunithm.beerpsi.cc/invite",
        INTER_32,
        spacing=12,
    )
    ...
    total_rating = floor(sum((item.extras[KEY_PLAY_RATING] for item in records), 0))
    ...
    if new_records is None:
        new_total_rating = 0
        rating_text = f"BEST 50 SUM {total_rating}"
    else:
        new_total_rating = floor(
            sum((item.extras[KEY_PLAY_RATING] for item in new_records), 0),
        )
        rating_text = f"OLD {total_rating} / NEW {new_total_rating}"
    ...
    if current_rating is not None:
        current_rating = total_rating + new_total_rating
        rating_text = f"RATING {current_rating} / {rating_text}"
    ...
    updated_text = "Generated at 2025-04-01"
...
class RecordsCog(commands.Cog, name="Records"):
    ...
    async def _scores_inner(
        ...
        *,
        ...
        async with ctx.typing():
            ...
            # the song is available in maimai DX International.
            #
            ...
            if len(songs) > 1:
                ...
                msg = f"No songs currently available in maimai DX International matches the search criteria. Closest match was **{escape_markdown(result.songs[0].title)}**."
                raise commands.BadArgument(msg)
            ...
            if kamaitachi:
                ...
                async with self.utils.chuninet(target_id) as client:
                    ...
                    message = await ctx.reply(
                        content="Fetching scores...", mention_author=False
                    )
                    ...
                    records = await self.utils.hydrate_records(records)
                    records.sort(key=lambda r: r.extras[KEY_PLAY_RATING], reverse=True)

                    new_records = [
                        r
                        for r in records
                        if r.extras[KEY_SONG_VERSION] == CURRENT_CHUNITHM_VERSION
                    ][:20]
                    new_record_slots = 20
    ...
    async def recent10(
        ...
        msg = "In my world, there is no recent rating. Soon, the penguin will also come to my side..."
        raise commands.CommandError(msg)
